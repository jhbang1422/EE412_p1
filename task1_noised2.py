...
# Keep the indices of each hand
# e.g., hands_index[X] is a list of data_points whose value is X.
hands_index = [[], [], [], [], [], [], [], [], [], []]
#print(data[0][52])
for i in range(len(data)):
    hands_index[int(data[i][52])].append(i)

#print(hands_index[8])
#print(len(hands_index)) # = 10
number_of_samples_to_noise = 10
noise_std = 99
#print(len(hands_index))
if flags.FLAGS.mode == "train":
    for i in range(len(hands_index)): # Don't need to over-sample Hand 0. There are enough data points already.
        sample_indices = np.random.randint(0, len(hands_index[i]), size=number_of_samples_to_noise)
   
        print("Generating synthetic, noised samples based on the training dataset of Hand " + str(i) + "...")
        for j in range(number_of_samples_to_noise):
            synthetic_data = [None]
            #for k in range(i*i*i + 100):
            for k in range(i*i*i + 100):
                noise = noise_std * np.random.randn(53)
                noise[52] = 0.0
                #print(data[hands_index[i][sample_indices[j]]])
                synthetic_data[0] = data[hands_index[i][sample_indices[j]]] + noise
                #print(synthetic_data[0])
                data = np.concatenate((data, synthetic_data))
