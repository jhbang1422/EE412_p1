...
def five_hot_code(vector, num_cards=52):
...
with tf.Session() as sess:
    ...
    for i in range(len(test_dat)):
        ...
        card_idx = find_index(list[max_idx])
        write=[]
        for j in range(len(card_idx)):
            write.append(card_idx[j]//13) # shape of each card 
            write.append(card_idx[j]%13) # card number 
        write.append(max_val)
        wr.writerow(write)
